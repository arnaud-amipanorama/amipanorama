  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, pointerEvents: "none" }}>
      ...
      {spotlight && <motion.div initial={false} animate={spotlight} transition={{ type: "spring", stiffness: 280, damping: 30 }}
        style={{ position: "fixed", borderRadius: 17, border: "2px solid #0F6B6D", boxShadow: "0 0 0 6px rgba(15,107,109,0.18), 0 0 32px rgba(15,107,109,0.42)", pointerEvents: "none" }} />}
      <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28 }}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <span style={{ color: "#0F6B6D", fontSize: 10.5, fontWeight: 800, letterSpacing: "0.14em" }}>PRISE EN MAIN GUIDÉE</span>
        ...
        <motion.div key={current.number} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.18 }}>
          <div style={{ color: "#0F6B6D", fontSize: 29, fontWeight: 800, letterSpacing: "-0.05em", lineHeight: 1 }}>{current.number}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5, margin: "22px 0 17px" }}>
          {steps.map((item, index) => <span key={item.number} style={{ display: "block", height: 3, flex: 1, borderRadius: 20, background: index <= step ? "#0F6B6D" : "rgba(255,255,255,0.13)", transition: "background 0.2s" }} />)}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
          ...
          <button type="button" onClick={() => last ? onComplete() : goToStep(step + 1)}
            style={{ border: 0, borderRadius: 9, background: "#0F6B6D", color: "#fff", cursor: "pointer", padding: "11px 15px", fontSize: 13, fontWeight: 700 }}>{last ? "Ouvrir le simulateur →" : "Voir l’étape suivante →"}</button>
        </div>
      </motion.section>
    </div>
  );
