    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, pointerEvents: "none" }}>
      ...
      <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28 }}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <span style={{ color: "#E85835", fontSize: 10.5, fontWeight: 800, letterSpacing: "0.14em" }}>PRISE EN MAIN GUIDÉE</span>
        ...
        <motion.div key={current.number} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.18 }}>
          <div style={{ color: "#E85835", fontSize: 29, fontWeight: 800, letterSpacing: "-0.05em", lineHeight: 1 }}>{current.number}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5, margin: "22px 0 17px" }}>
          {steps.map((item, index) => <span key={item.number} style={{ display: "block", height: 3, flex: 1, borderRadius: 20, background: index <= step ? "#E85835" : "rgba(255,255,255,0.13)", transition: "background 0.2s" }} />)}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
          ...
          <button type="button" onClick={() => last ? onComplete() : goToStep(step + 1)}
            style={{ border: 0, borderRadius: 9, background: "#E85835", color: "#fff", cursor: "pointer", padding: "11px 15px", fontSize: 13, fontWeight: 700 }}>{last ? "Ouvrir le simulateur →" : "Voir l’étape suivante →"}</button>
        </div>
      </motion.section>
    </div>
  );
