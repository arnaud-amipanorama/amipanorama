  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 2000, pointerEvents: "none" }}>
      ...
      {spotlight && <motion.div initial={false} animate={spotlight} transition={{ type: "spring", stiffness: 280, damping: 30 }}
        style={{ position: "fixed", borderRadius: 17, border: "2px solid #9DBBFF", boxShadow: "0 0 0 6px rgba(157,187,255,0.18), 0 0 32px rgba(118,163,255,0.34)", pointerEvents: "none" }} />}
      <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.28 }}
        ...
        style={{ position: "fixed", pointerEvents: "auto", right: "clamp(16px, 4vw, 42px)", top: last ? "clamp(16px, 4vw, 42px)" : "auto", bottom: last ? "auto" : "clamp(16px, 4vw, 42px)", width: "min(calc(100% - 32px), 430px)", maxHeight: "calc(100vh - 32px)", overflowY: "auto", padding: "24px", borderRadius: 18, background: "#1B2433", border: "1px solid #53647E", boxShadow: "0 24px 80px rgba(0,0,0,0.55)", color: "#F8FAFC" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <span style={{ color: "#9DBBFF", fontSize: 10.5, fontWeight: 800, letterSpacing: "0.14em" }}>PRISE EN MAIN GUIDÉE</span>
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <button type="button" onClick={onComplete} style={{ border: 0, padding: 0, background: "transparent", color: "#D5DDEA", cursor: "pointer", fontSize: 12 }}>Passer le tutoriel</button>
            <span style={{ color: "#D5DDEA", fontSize: 12 }}>{step + 1} / {steps.length}</span>
          </div>
        </div>
        <motion.div key={current.number} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.18 }}>
          <div style={{ color: "#9DBBFF", fontSize: 29, fontWeight: 800, letterSpacing: "-0.05em", lineHeight: 1 }}>{current.number}</div>
          <h1 id="sim-onboarding-title" style={{ margin: "11px 0 7px", fontSize: 23, lineHeight: 1.1, letterSpacing: "-0.035em" }}>{current.title}</h1>
          <p style={{ margin: 0, color: "#D5DDEA", fontSize: 13.5, lineHeight: 1.58 }}>{current.text}</p>
        </motion.div>
        <div style={{ display: "flex", alignItems: "center", gap: 5, margin: "22px 0 17px" }}>
          {steps.map((item, index) => <span key={item.number} style={{ display: "block", height: 3, flex: 1, borderRadius: 20, background: index <= step ? "#9DBBFF" : "rgba(255,255,255,0.20)", transition: "background 0.2s" }} />)}
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
          ...
          <button type="button" onClick={() => last ? onComplete() : goToStep(step + 1)}
            style={{ border: 0, borderRadius: 9, background: "#3E68BE", color: "#fff", cursor: "pointer", padding: "11px 15px", fontSize: 13, fontWeight: 700 }}>{last ? "Ouvrir le simulateur →" : "Voir l’étape suivante →"}</button>
        </div>
      </motion.section>
    </div>
  );
